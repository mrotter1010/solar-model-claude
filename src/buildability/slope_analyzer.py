...
def analyze_slope(
    ...
    # Summary statistics
    min_deg = float(np.min(slope_array))
    max_deg = float(np.max(slope_array))
    mean_deg = float(np.mean(slope_array))
    median_deg = float(np.median(slope_array))

    # Bucketed distribution
    distribution = _compute_distribution(slope_array, thresholds, total_pixels)

    # Suitability percentages
    pct_tracker = float(np.sum(slope_array < _TRACKER_SLOPE_LIMIT) / total_pixels * 100)
    pct_fixed = float(np.sum(slope_array < _FIXED_TILT_SLOPE_LIMIT) / total_pixels * 100)
